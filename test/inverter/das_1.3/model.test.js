...
describe('Decoding Test', function() {
  ...
  it('receive Buffer To Data Body', function(done) {
    ...
    let bodySystem =  model.getValidateData(dummySystem, protocol.SYSTEM);
    BU.CLI('bodySystem', bodySystem);
    
    
    let dummyPv = Buffer.from('^D120001,400,0200,0080,18');
    let bodyPv =  model.getValidateData(dummyPv, protocol.PV);
  ...
  it.only('automaticDecoding', function(done) {
    const converter = new Converter({deviceId:'001',mainCategory: 'Inverter', subCategory: 'das_1.3', option: true, protocolOptionInfo:{hasTrackingData: true}} );
    // 명령 생성
